...
void DungeonMap::clear()
{
    ...
    for (Map<Vector2, Region*>::Element* e = regions.front(); e; e = e->next()) {
        ...
        if (region->mesh_instance) {
            region->mesh_instance->queue_delete();
        }

        if (region->edge_mesh_instance) {
            region->edge_mesh_instance->queue_delete();
        }
...
Vector2 DungeonMap::get_tile_position_from_vec3(const Vector3& position)
{
    return get_tile_position(Vector2(position.x, position.z));
}
...
void DungeonMap::set_map_tile_color(const Vector3& position, Color color)
{
    Vector2 tile_id = get_tile_position_from_vec3(position);
    if (color == Color(1.0f, 1.0f, 1.0f)) {
        Tile* tile = find_tile(tile_id);
        if (tile == nullptr) {
            map_builder.set_map_tile_color(tile_id, color);
            return;
        }
        map_builder.set_map_tile_color(tile_id, tile->color);
    } else {
        map_builder.set_map_tile_color(tile_id, color);
    }
}
...
void DungeonMap::mark_tile_occupied(const Vector2& position, bool occupied)
{
    Tile* tile = find_tile(get_tile_position(position));
    if (tile == nullptr) {
        return;
...
bool DungeonMap::is_tile_occupied(const Vector2& position)
{
    Tile* tile = find_tile(get_tile_position(position));
    if (tile == nullptr) {
        return false;
...
void DungeonMap::_update_tile_neighbors()
{
    ...
    for (Map<Vector2, Tile*>::Element* e = tiles.front(); e; e = e->next()) {
        ...
        if (!tile->is_edge && tile->type == FLOOR) {
            valid_tiles.push_back(tile->id);
        }
    }
}
...
void DungeonMap::_build_region_meshes()
{
    for (Map<Vector2, Region*>::Element* e = regions.front(); e; e = e->next()) {
        ...
        this->add_child(region->mesh_instance);
        region->mesh_instance->set_owner(this);
...
void DungeonMap::_build_region_mesh_edges()
{
    for (Map<Vector2, Region*>::Element* e = regions.front(); e; e = e->next()) {
        ...
        for (int i = 0; i < region->tiles.size(); i++) {
            ...
            DungeonMapMeshBuilder::add_mesh_tile_edge(this, &tool, region->tiles[i], ceiling_height, false);
        }
        tool.generate_normals();
        tool.index();
        ...
        this->add_child(region->edge_mesh_instance);
        region->edge_mesh_instance->set_owner(this);
...
void DungeonMap::_create_collision(StaticBody*& collision_body, CollisionShape*& collision_shape, Ref<Mesh> mesh, const Transform& transform)
{
    ...
    collision_body->set_meta("valid_hit_target", true);
}
...
void DungeonMap::_create_region_collision(const Vector2& region_id)
{
    ...
    region->collision_body->set_meta("is_floor", true);
}
...
void DungeonMap::_create_region_edge_collision(const Vector2& region_id)
{
    ...
    region->collision_body->set_meta("is_wall", true);
}
...
void DungeonMap::_update_visibility()
{
    ...
    for (auto e = regions.front(); e; e = e->next()) {
        ...
        if (region->mesh_instance) {
            if (is_visible()) {
                region->mesh_instance->show();
            } else {
                region->mesh_instance->hide();
            }
        }

        if (region->edge_mesh_instance) {
            if (is_visible()) {
                region->edge_mesh_instance->show();
            } else {
                region->edge_mesh_instance->hide();
            }
        }
...
void DungeonMap::_bind_methods()
{
    ...
    ClassDB::bind_method(D_METHOD("set_show_ceiling", "show"), &DungeonMap::set_show_ceiling);
    ClassDB::bind_method(D_METHOD("get_show_ceiling"), &DungeonMap::get_show_ceiling);

    ClassDB::bind_method(D_METHOD("mark_tile_occupied", "position", "occupied"), &DungeonMap::mark_tile_occupied);
    ClassDB::bind_method(D_METHOD("is_tile_occupied", "position"), &DungeonMap::is_tile_occupied);

    ClassDB::bind_method(D_METHOD("set_tile_color", "position", "color"), &DungeonMap::set_tile_color);
