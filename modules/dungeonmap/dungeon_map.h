...
public:
    ...
    struct Region
    {
        ...
        MeshInstance* edge_mesh_instance = nullptr;
...
private:
    ...
    // Celling height.
    int ceiling_height = 4;

    // Flag for whether or not the dungeon is dirty.
    bool dirty = true;

    // Flag to show the ceiling.
...
public:
    ...
    // Returns a tile id.
    Vector2 get_tile_position_from_vec3(const Vector3& position);

    // Checks if a position is valid.
    bool is_valid_position(const Vector2& position);

    // Set the map image color for a specific tile.
    void set_map_tile_color(const Vector3& position, Color color);
    ...
    void mark_tile_occupied(const Vector2& position, bool occupied);

    // Returns whether or not a tile is occupied.
    bool is_tile_occupied(const Vector2& position);

    // Save the tile color so that it always returns to this color.
    // This is permenant whereas set_map_tile_color is meant to
    // be temporary.
    _FORCE_INLINE_ void set_tile_color(const Vector3& position, Color color) {
    ...
    // Sets the show ceiling flag.
    _FORCE_INLINE_ void set_show_ceiling(bool show) { show_ceiling = show; }
    // Returns the show ceiling flag.
    _FORCE_INLINE_ bool get_show_ceiling() const { return show_ceiling; }
};
